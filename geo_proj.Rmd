...
plot(ok.var)
## zwykły kriging
ok.xy <- autoKrige(ruch~1, input, output, verbose=FALSE)
```
...
```{r, fig.cap = 'Figure 4. Variograms for the kriging models.'}
...
avg_odl_trunk + min_odl_primary + max_odl_primary + avg_odl_primary + od_centrum + 
od_stacji_min + od_stacji_max + ile_stacji_r5 + ile_stacji_r10, input, output, verbose=FALSE)
plot(uk.var.1)
```
...
res_rf_kmeans<-sperrorest(modelformula, data=dane, 
                          ...
                          model_args=list(ntree=450, mtry=1), 
